...
(function($root) {
  ...
  var Phenotype = {
     ...
     *
     *  The Phenotype module manages a cell's actual observed properties, such as textContent ($text), nodeType ($type), innerHTML ($html), childNodes ($has), and DOM attributes
     *
     ...
     *   - $has(): translates the "$has" attribute to childNodes
    ...
    $has: function($parent, components) {
      ...
      if (old.length > 0) {
        ...
        diff['+'].forEach(function(item) {
          ...
          $parent.$has[item.index] = $parent.childNodes[item.index].Genotype;
        });
      } else {
        ...
        $parent.$has = [].map.call($parent.childNodes, function($node) { return $node.Genotype; });
      }
    },
    $init: function($node) {
  ...
  var Nucleus = {
    ...
    build: function($node) {
      // 1. The special attributes "$type", "$text", "$html", "$has" are tracked by default even if not manually defined
      ['$type', '$text', '$html', '$has'].forEach(function(key) {
